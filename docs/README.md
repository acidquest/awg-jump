...
- Домены из списка в веб-интерфейсе (страница **Split DNS**) резолвятся через Яндекс DNS (`77.88.8.8`).
- Все остальные домены идут через `1.1.1.1` / `8.8.8.8`.
- DNS-запросы самого контейнера маршрутизируются по тем же правилам GeoIP: запросы к российским IP — через `eth0`, к остальным — через `awg1` (upstream VPN).

**Управление доменами:**

Страница **Split DNS** в веб-интерфейсе позволяет добавлять, отключать и удалять домены. При старте создаётся набор дефолтных RU-доменов (`ru`, `рф`, `yandex.ru`, `vk.com`, `mail.ru`, `sber.ru`, `gosuslugi.ru` и др.).

Список доменов сохраняется в `config.db` и включается в бэкап автоматически.
