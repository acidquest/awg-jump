...
| Переменная | По умолчанию | Описание |
|-----------|-------------|----------|
| `AWG0_LISTEN_PORT` | `51820` | UDP-порт AWG-сервера |
| `AWG0_PRIVATE_KEY` | _(авто)_ | Приватный ключ; если пусто — генерируется автоматически |
| `AWG0_ADDRESS` | `10.10.0.1/24` | Адрес интерфейса awg0 |
| `AWG0_DNS` | _(awg0 IP)_ | DNS для клиентов awg0 |
| `CLASSIC_WG` | _(пусто)_ | `on` включает дополнительный интерфейс `wg0` |
| `WG0_LISTEN_PORT` | _(обязателен при `CLASSIC_WG=on`)_ | UDP-порт classic WireGuard сервера |
| `WG0_PRIVATE_KEY` | _(авто)_ | Приватный ключ wg0 |
| `WG0_ADDRESS` | `10.11.0.1/24` | Адрес интерфейса wg0 |
| `WG0_DNS` | `10.11.0.1` | DNS для клиентов wg0 |
...
Управление туннельными интерфейсами `awg0`, `awg1` и опциональным `wg0`:
...
Управление клиентами серверных интерфейсов:
