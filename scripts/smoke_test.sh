...
text = text.replace("WEB_PORT=8080", "WEB_PORT=8443")
...
echo "[smoke] curl -kf https://localhost:${WEB_PORT}/api/system/status"
curl -kf "https://localhost:${WEB_PORT}/api/system/status"

echo "[smoke] curl -kf -X POST https://localhost:${WEB_PORT}/api/auth/login -d '{\"username\":\"admin\",\"password\":\"test\"}'"
curl -kf -X POST "https://localhost:${WEB_PORT}/api/auth/login" \
