...
`AWG Gateway` в каталоге `gateway/` — это автономный Linux-only шлюз внутри репозитория `awg-jump`. Он предназначен для сценария, где сам входной узел уже существует, а шлюзу нужно:
...
`gateway` не меняет входную ноду и не строит поверх туннеля полноценный двусторонний control plane. При этом он может отправлять в `awg-jump` свой client status (`1001`) после подключения и далее раз в 10 минут, если entry node peer `.conf` был экспортирован из актуального `awg-jump` и status endpoint доступен внутри туннеля.
...
5. Если `.conf` был выгружен из нового `awg-jump`, в нём есть metadata-комментарий с tunnel URL status API. После успешного подключения `gateway` пытается отправить туда свой код клиента `1001`, а затем повторяет отчёт каждые 10 минут.
