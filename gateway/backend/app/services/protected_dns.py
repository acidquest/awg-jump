...
def runtime_dir() -> Path:
    return Path(settings.dns_runtime_dir) / "protected"


def stubby_config_path() -> Path:
    return runtime_dir() / "stubby.yml"


def cloudflared_config_path() -> Path:
    return runtime_dir() / "cloudflared.yml"
...
def _terminate_process(proc: subprocess.Popen | None, name: str) -> None:
    ...
    try:
        ...
        logger.warning("[gateway-dns] %s did not stop gracefully, sending SIGKILL", name)
        proc.kill()
        proc.wait(timeout=3)
...
def _start_stubby(*, host: str, port: int, bootstrap_address: str) -> None:
    ...
    _STUBBY_PROCESS = subprocess.Popen(
        ["stubby", "-C", str(stubby_config_path())],
...
def _start_cloudflared(*, endpoint_url: str, bootstrap_address: str) -> None:
    ...
    ensure_runtime_dir()
    cloudflared_config_path().write_text(
    ...
    _CLOUDFLARED_PROCESS = subprocess.Popen(
        ["cloudflared", "proxy-dns", "--config", str(cloudflared_config_path())],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.PIPE,
        text=True,
    )
...
def status() -> dict:
    return {
        ...
        "cloudflared": {
            ...
            "config": str(cloudflared_config_path()),
        },
    }
