...
async def collect_system_metrics(session: AsyncSession) -> SystemMetric:
    ...
    previous = await session.scalar(select(SystemMetric).order_by(SystemMetric.collected_at.desc()).limit(1))
    ...
    cutoff = (datetime.now(timezone.utc) - timedelta(hours=RETENTION_HOURS)).replace(tzinfo=None)
