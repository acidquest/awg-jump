...
6. [AmneziaWG и обфускация](#amneziawg-и-обфускация)
7. [GeoIP и маршрутизация](#geoip-и-маршрутизация)
8. [Split DNS](#split-dns)
9. [Upstream-ноды и failover](#upstream-ноды-и-failover)
10. [Бэкап и восстановление](#бэкап-и-восстановление)
11. [TLS и Nginx](#tls-и-nginx)
...
Дополнительно предоставляет **Split DNS**: клиенты получают встроенный DNS-сервер, который направляет запросы к российским доменам через Яндекс DNS, а остальные — через Cloudflare/Google.
...
Браузер (HTTPS:443)
    ...
    ▼
┌─────────────┐
│    nginx    │  TLS termination, self-signed cert (10 лет)
│  :443/:80   │
└──────┬──────┘
       │ HTTP proxy
       ▼
...
Клиент → awg0 → iptables mangle PREROUTING:
    dst в ipset geoip_ru  →  fwmark RU  →  table 100  →  eth0 (прямой)
    dst не в geoip_ru     →  fwmark VPN →  table 200  →  awg1 (VPN)

Контейнер (DNS-запросы dnsmasq) → iptables mangle OUTPUT:
    dst в ipset geoip_ru  →  fwmark RU  →  table 100  →  eth0
    dst не в geoip_ru     →  fwmark VPN →  table 200  →  awg1
```
...
- Docker Engine 24+
- docker compose v2
- Открытые порты: `443/tcp` (HTTPS), `80/tcp` (HTTP→HTTPS), `51820/udp` (AWG)
...
5. Загружает GeoIP-кэш в ipset `geoip_ru`.
...
|-----------|-------------|----------|
| `NGINX_HTTPS_PORT` | `443` | Внешний HTTPS-порт |
| `NGINX_HTTP_PORT` | `80` | HTTP-порт (редирект на HTTPS) |
...
| Переменная | По умолчанию | Описание |
|-----------|-------------|----------|
| `GEOIP_SOURCE_RU` | ipdeny.com | URL файла с RU CIDR-блоками |
...
Открывается по `https://<SERVER_HOST>:<NGINX_HTTPS_PORT>`. Все страницы требуют авторизации.
...
- Статус ipset `geoip_ru`: количество префиксов, дата обновления.
- Источник данных (ipdeny.com).
- Ручное обновление GeoIP.

### Split DNS

- Статус dnsmasq: запущен/остановлен, PID, адрес прослушивания.
- Список доменов: домен, upstream (Yandex / Default), включён/выключен.
...
- Скачивание ZIP-архива с `config.db` (все данные) + `env_snapshot.json` + `wg_configs/`.
...
1. При старте (и по расписанию `GEOIP_UPDATE_CRON`) загружается файл RU CIDR-блоков с `GEOIP_SOURCE_RU`.
2. Блоки загружаются в ipset `geoip_ru` (atomic swap — без разрыва соединений).
3. iptables mangle **PREROUTING** маркирует входящие от `awg0` пакеты:
   - dst в `geoip_ru` → `fwmark RU` → таблица 100 → `eth0`
   - dst не в `geoip_ru` → `fwmark VPN` → таблица 200 → `awg1`
4. iptables mangle **OUTPUT** маркирует трафик самого контейнера (DNS-запросы и т.д.) по тем же правилам.
5. `iptables nat POSTROUTING MASQUERADE` обеспечивает NAT на обоих исходящих интерфейсах.
...
Клиент → AWG DNS (10.10.0.1:53) → dnsmasq:
    домен в списке (upstream=yandex) → 77.88.8.8 (Яндекс DNS)
    всё остальное                   → 1.1.1.1, 8.8.8.8
```

DNS-трафик самого контейнера (запросы dnsmasq к upstream DNS) маршрутизируется через iptables OUTPUT — `77.88.8.8` идёт через `eth0` (Russian IP в geoip_ru), `1.1.1.1` и `8.8.8.8` — через `awg1`.
...
Значения `upstream`:
- `yandex` — резолвить через `77.88.8.8`
- `default` — резолвить через `1.1.1.1` / `8.8.8.8` (явно переопределить для поддомена)

### Перезагрузка конфига

При любом изменении доменов конфиг dnsmasq перегенерируется автоматически и перезагружается через `SIGHUP` (без прерывания соединений).
...
| Содержимое | Описание |
|-----------|----------|
| `config.db` | Все данные: интерфейсы, ключи, обфускация, пиры, ноды, GeoIP-источники, правила маршрутизации, **split DNS домены** |
| `env_snapshot.json` | Справочный снимок публичных параметров (без паролей) |
| `wg_configs/` | Сгенерированные WireGuard конфиги |

**Что НЕ включено:**
- TLS-сертификаты (авто-генерируются при старте)
- GeoIP-кэш (перезагружается по крону)
...
Nginx выступает TLS-терминатором. Бэкенд `awg-jump` работает по HTTP внутри docker-сети и снаружи **не публикуется**.

### Self-signed сертификат

Генерируется скриптом `nginx/generate-cert.sh` при первом старте:
...
│   └── generate-cert.sh    # Авто-генерация self-signed cert
│
...
│   │   ├── geoip_fetcher.py # Загрузка GeoIP
