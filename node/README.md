...
3. На VPS с нодой сгенерируй приватный ключ для `AWG_PRIVATE_KEY` и выбери адрес интерфейса ноды, например `10.20.0.3/32`. В managed-деплое адрес и сеть туннеля задаются в карточке ноды и не берутся из `.env` jump-сервера.
...
После активации `awg-jump` обновит endpoint интерфейса `awg1` и начнёт слать основной VPN-трафик через эту ноду. Если нода включена как GeoIP-нода, она используется отдельным туннелем `awg2` для префиксов local routing zone; исключения из этой зоны остаются на прямом физическом интерфейсе.
